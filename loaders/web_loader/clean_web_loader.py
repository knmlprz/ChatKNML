...
class CleanWebLoader(BaseLoader):
    ...
    def junk_remover(docs):
        ...
        """
        short_docs = [doc for doc in docs if len(doc.page_content) < 300]
        junk_docs = list(short_docs)
        seen_texts = set()
        for doc in docs:
            if doc.page_content in seen_texts and doc not in junk_docs:
                junk_docs.append(doc)
            elif 'title' not in doc.metadata.keys():
                junk_docs.append(doc)
            else:
                seen_texts.add(doc.page_content)
        return [doc for doc in docs if doc not in junk_docs]
    ...
    def load(self) -> List[dict]:
        ...
        for address in self.url_list:
            try:
                loader = RecursiveUrlLoader(url=address, max_depth=self.depth, 
                                            extractor=CleanWebLoader.newspaper_extractor)
                docs.extend(loader.load())
            except Exception as e:
                print(f"Exception: {e}")
                break
        docs = reduce(lambda data, method: method(data), [CleanWebLoader.junk_remover, CleanWebLoader.ds_converter], docs)
        return docs
    
    def load_and_split(self, text_splitter: Optional[TextSplitter] = None, chunk: int = 400, chunk_overlap: int = 80) -> List[dict]:
        """
        Loads web content from specified URLs, extracts text using the 'newspaper' library,
        ...
        if text_splitter is None:
            _text_splitter: TextSplitter = SpacyTextSplitter(pipeline='pl_core_news_sm', chunk_size=chunk, chunk_overlap=chunk_overlap)
        else:
